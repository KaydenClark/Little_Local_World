...
|---|---|---|
| Writable roots (`src/agent_town/`, `tests/`, `scripts/`, `docs/`, control docs) | `allow` | edits run without a prompt |
| Forbidden paths (secrets, `.venv/`, `logs/`, build output, asset zips) | `deny` | hard-blocked; takes precedence over `allow` |
...
- The `Bash(...)` allow entries assume `PYTHONPATH=src` and the project venv; see
  `RUNBOOK.md` for the exact per-platform invocation.
