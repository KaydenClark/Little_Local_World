...
This file controls how agents behave in this project. It answers four
questions: what can the agent read, what can it edit, how does it choose work,
and where is the proof that the work is done.
...
- this project root (`E:\GPTCode\local-agent-town`, or the equivalent checkout
...
- external paths only when the user request or project docs explicitly reference
  them, such as `E:\GPTCode\LLM_Workbench`.
...
- `tests/`;
- `scripts/`;
- `docs/screenshots/` when updating checked-in visual proof, `docs/proof/<slice>/`
  when recording watchability proof, and `docs/run_reports/` when recording run
  gates;
- control docs: `AGENTS.md`, `BLUEPRINT.md`, `TASKBOARD.md`, `RUNBOOK.md`,
  `README.md`, `HARNESS_FEEDBACK.md`, `VISUAL_DESIGN.md`, `BRANCHING.md`;
...
- Merge feature branches into `integration` with a merge commit (do not squash);
  the merge history is the record of who did what. Delete a feature branch after
  it merges - its commits are preserved in `integration`.
- Deleting branches already merged into `integration` or `main` is safe and
  expected cleanup. Never delete or force-push `main` or `integration`.
...
| Branch naming, merge flow, cleanup rules | `BRANCHING.md` |
...
complete until its new state is proven **visible and correct on screen**, not
only green in headless tests. Headless tests prove the core; they do not prove
the spectator experience.

- Render a real frame of the new behavior and **look at it** before claiming
  done: set `SDL_VIDEODRIVER=dummy`, build the state, call
  `render_civilization(...)`, `pygame.image.save(...)`, then open the image.
  Save proof under `docs/proof/<slice>/`.
...
- Add a viewer test that asserts the new signal surfaces with the right priority
  (a survival-staple shortage must outrank lesser warnings), so a buried signal
  is a red test, not a silent gap.
- If the change involves the local model, verify the model path **actually
  ran** - a model loaded in LM Studio, a non-fallback decision outcome - before
  claiming the LLM behavior works. The fallback governor needs no model and no
  GPU, so a green fallback run proves nothing about the model path. Say so
  plainly if the model path was not exercised.
...
- Create a dedicated branch for non-trivial work (see Branching And Merge Flow
  above). Do not overwrite, reset, discard, or force-push user work unless
  explicitly asked. Inspect git state before editing; keep unrelated dirty files
  untouched.
...
- Create a checkpoint before broad edits, before dependency changes, after
  verification passes, and before handoff. Each checkpoint records: current
  goal, files changed, verification run, known risks, and next action.
...
agent's committed work - if a branch exists, continue from it or record why you
are not.
...
- keep original downloaded assets/zips untouched and derive runtime sheets from
  them;
- match the target mood (medieval fantasy village; default to
  Age-of-Empires-style readable RTS buildings/terrain when uncertain);
...
run baseline verification when practical, implement with tests or a named
manual check, and append a `TASKBOARD.md` proof row if durable state changed.
...
- Keep the raw research files intact unless the user explicitly asks to rename
  or reorganize them.
- Do not paste long research sections into active docs. Extract only the
  project rule, affected files, tests, risks, and deferrals.
- If research conflicts with current implementation, record the conflict in
  `TASKBOARD.md` and either implement a small reconciliation slice or leave an
  explicit next task.
...
- Do not treat prior session notes or taskboard history as current truth
  without verifying source state.
