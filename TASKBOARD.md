...
|---|---|---|---|---|---|---|
| B-001 | Sandbox git write operations in this repo | Resolved after stale `index.lock` cleanup | Local checkpoint commit and merge completed on `recovery/llw-local-work-20260716` | Keep stale-lock cleanup as the recovery procedure; no owner action required | Kayden | resolved |
...
| 2026-07-16 | (repository-reconciliation) | codex | `tests.test_llm` 10/10; headless smoke exit 0; Workbench validation passed; merged `origin/adopt/llm-workbench-v2.1` | recovery branch and remote branch | pass | Preserved local LM work, archived controls, run reports, and proof images; reconciled remote adoption history | Full 262+ test discovery was not rerun for this repository-only checkpoint |
