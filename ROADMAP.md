...
| 2026-06-29 | Run-log & monitoring system: telemetry JSONL + invariants/health + in-game History feed + `analyze_run.py` | `.\.venv\Scripts\python.exe -m unittest discover -s tests` (198 tests, was 174); `.\.venv\Scripts\python.exe -m agent_town --smoke-test`; `.\scripts\validate-workbench.ps1`; offline `scripts\llm_governor_run.py --hours 30` -> `logs\run-*.jsonl` then `scripts\analyze_run.py` (OK exit 0; a stressed log exits 1); rendered `docs\screenshots\monitoring.png` | pass | Added `telemetry.py` (record builders for run_start/snapshot/decision/event/run_end; `JsonlFileSink`/`RingBufferSink`/`MultiSink`/`NullSink`; `RunLogger`; `TelemetryGovernor` pass-through wrapper) and `health.py` (`check_invariants` conservation/one-job-per-pawn guard; edge-triggered `detect_events`; `health_summary`/`run_problems`). Logging is a pure hour-boundary observer - a test proves logging on/off yields identical sim state; wall-clock lives only in run_start/run_end. `LLMGovernor` gained telemetry-only `last_outcome`/`last_error` (model/fallback/error/disabled) so dropped decisions are detectable; "disabled" (no model) is not counted as dropped. Viewer wires a `RunLogger` (events ring + JSONL file in real runs), wraps its governor in `TelemetryGovernor`, adds the History event-feed panel (History button, mutually exclusive with Work), and a latched HUD `ALERT` dot. `scripts/llm_governor_run.py` now writes a run log; new `scripts/analyze_run.py` prints a health report and gates with a non-zero exit. `.gitignore` ignores `logs/`. New `test_telemetry.py` + `test_health.py` (+viewer feed tests). Determinism oracles unaffected. |
| 2026-06-29 | Tune local-LLM governor defaults for longer work-priority JSON | RED: `.\.venv\Scripts\python.exe -m unittest tests.test_llm.LocalLLMClientTests.test_from_env_uses_safe_governor_response_defaults` failed on old `4.0` timeout; GREEN: `.\.venv\Scripts\python.exe -m unittest tests.test_llm` (7 tests); `.\.venv\Scripts\python.exe -m unittest discover -s tests` (175 tests); `.\.venv\Scripts\python.exe -m agent_town --smoke-test`; `.\scripts\validate-workbench.ps1`; `git diff --check` | pass | Raised default local-LLM request budget from 4s/180 tokens to 8s/320 tokens so double-click and normal launches have enough room for compact `set_work_priority` action lists after the latest Gemma run clipped one response at length. Updated RUNBOOK defaults, troubleshooting, and manual PowerShell launch instructions. Remaining: rerun a live LM Studio viewer/headless session and inspect whether `finish_reason: "length"` disappears in the server log. |
