...
| 2026-06-26 | Prune pointless/redundant tests and harden weak ones (mutation-tested) | `python3 -m unittest discover -s tests` (45 tests); verified 4 deliberate code mutations each turn exactly one test red; `python3 -m agent_town --smoke-test` | pass | Removed 2 tautology/redundant tests (`_location_color` has a `.get` fallback so its range check could never fail; a kinds-set test duplicated the per-building kind test) and 2 agent-loop tests that re-ran agent-agnostic logic already covered in `test_core`; made the walking-paths test bite the real `WALKING_PATHS` constant (extracted from `_draw_paths`) instead of a copy; tightened the camera-clamp test to pin the actual viewport margin; added missing coverage for `_validate_initial_state` (empty-locations raise, bad-destination raise, home/workplace coercion) and the `apply_decision` happy path |
