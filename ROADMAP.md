...
Per user direction (2026-06-29), the observer/run-log slice landed before
water: structured JSONL logs, pure health checks, analyzer exit codes, and the
in-game History feed now make run behavior inspectable live and after the fact.
...
5. **Civilization rename, the RimWorld mood foundation, and the Civ stats bar
   ...
   3. **(done) Civ stats bar.** `economy.average_need(state, need)` gives the
      ...
      top-left translucent Civ stats panel (Mood + Food/Recreation/Rest readouts)
      reusing the need-bar colours. Build 1 only adds a mood consequence for food;
      the other three are readouts (the red Food bar makes the deliberate hunger
      pressure visible). Tests: `average_need` on a known roster + a panel render
      smoke check.
...
6. **Observer-first UI pass** -
   ...
   - Current status: the app has the first observer monitoring slice: structured
     JSONL run logs, health/invariant summaries, analyzer red/green exit codes,
     a History button/feed, and a HUD alert chip for warn/critical events. The
     Governor's current plan, bottleneck, and confidence card remain future work.
...
| 2026-06-29 | Run-log monitoring system: JSONL telemetry, live History feed, health checks, analyzer | `.\.venv\Scripts\python.exe -m unittest tests.test_telemetry tests.test_health tests.test_civilization_view` (37 tests); `.\.venv\Scripts\python.exe scripts\llm_governor_run.py --hours 2 --log /tmp/little-local-world-test-run.jsonl`; `.\.venv\Scripts\python.exe scripts\analyze_run.py /tmp/little-local-world-test-run.jsonl`; `.\.venv\Scripts\python.exe -m unittest discover -s tests` (188 tests, was 174); `.\.venv\Scripts\python.exe -m agent_town --smoke-test`; `.\scripts\validate-workbench.ps1`; `git diff --check`; refreshed `docs\screenshots\current-state.png` | pass | Added observer-only `telemetry.py` (JSONL file sink, ring buffer, multi/no-op sinks, `TelemetryGovernor`, run/hour/end records) and `health.py` (invariants, summaries, red conditions). The viewer wraps the live governor for logging, writes ignored `logs/run-*.jsonl`, reads the in-memory ring buffer for the `History` panel, and shows a HUD alert chip for warn/critical events without changing `engine.step_hour`. `scripts\llm_governor_run.py` now writes a JSONL log path, and `scripts\analyze_run.py` prints health summaries and exits non-zero on red conditions. Remaining gap: manual click-through in a real Pygame window and a live local-model success run were not performed in this pass; the verified runner path used the offline hard-fallback model state. |
| 2026-06-29 | PC-hosted Gemma partial-week proof for the run-log monitor | `AGENT_TOWN_LLM_BASE_URL=http://192.168.1.131:1234/v1 AGENT_TOWN_LLM_MODEL=google/gemma-4-e4b .\.venv\Scripts\python.exe scripts\llm_governor_run.py --hours 168 --log logs\week-proof-2026-06-29-gemma-4-e4b.jsonl` (stopped by request after 133 simulated hours); `.\.venv\Scripts\python.exe scripts\analyze_run.py logs\week-proof-2026-06-29-gemma-4-e4b.jsonl` | red as expected | The PC endpoint was reachable and the model applied 89 `set_work_priority` actions over 133 completed hours (day 5, 20:00). Analyzer handled the partial log and correctly failed the proof because bread depleted at day 2, hour 7 (`CRITICAL good_depleted`). No dropped decisions, no invariant violations, no breaks; mood start/min/end was 85.8/52.4/56.4, final food need was 0.0, final stockpile was grain 10. See `docs/run_reports/2026-06-29-gemma-4-e4b-partial-week.md`. This is evidence that the monitoring stack works and that the current food/governor loop is not yet healthy for a week-long autonomous run. |
