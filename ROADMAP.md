...
| 2026-06-28 | Prune pointless/redundant legacy wander-sim tests and harden weak ones (mutation-tested) | `python3 -m unittest discover -s tests` (106 tests, was 107); verified 4 deliberate code mutations each turn exactly one test red, then suite green again; `python3 -m agent_town --smoke-test` | pass | Scoped to the legacy agent/building/app tests (`test_app`, `test_buildings`, `test_default_agents`, `test_core`); did not touch the new colony-contract / Track A/B / persistence / integration suites. Removed 2 tests that could never fail (`_location_color` has a `.get` fallback so its range check is a tautology; a kinds-set test duplicated the per-building kind test) and 2 agent-loop tests that re-ran agent-agnostic logic already covered in `test_core`; extracted `WALKING_PATHS` from `_draw_paths` so the paths test bites the real constant instead of a copy; tightened the camera-clamp test to pin the actual viewport margin; added missing coverage for `_validate_initial_state` (empty-locations raise, bad-destination raise, home/workplace coercion) and the `apply_decision` happy path |
